...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"Guests: {len(GUESTS)}\n"
        f"Used: {len(USED)}\n\n"
        "/like 12345678 → Try likes\n"
        "Note: Free likes patched in 2025"
    )
...
async def like(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in available:
        try:
            ...
            # TRY MULTIPLE ENDPOINTS (ONE MIGHT WORK)
            urls = [
                "https://ff-like.garena.com/api/like",
                "https://like.freefiremobile.com/v1/like",
    ...
    await update.message.reply_text(
        ...
        f"If 0 → Free likes are patched in 2025"
    )
